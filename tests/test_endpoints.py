...
class TestEndpoints(unittest.TestCase):
    ...
    def test_get_all_orders(self):
        ...
        with self.app.test_request_context():
            self.client.post("/api/V1/menu", json={'foodname': 'Pizza', 'price': '$4.99'})
            self.client.post("/api/V1/orders", json={'foodname': 'Pizza', 'price': '$6.99'})
            res = self.client.get("/api/V1/orders")
    ...
    def test_get_specific_order(self):
        with self.app.test_request_context():
            ...
            self.assertEqual("The order was not found", res.get_data(as_text=True))
    ...
    def test_modifying_status(self):
        with self.app.test_request_context():
            ...
            res = self.client.put("/api/V1/orders/0", json={'status': 'accepted'})
            self.assertEqual("The order was not found", res.get_data(as_text=True))
    ...
    def test_adding_meal_to_menu(self):
        ...
        with self.app.test_request_context():
            ...
            self.client.post("/api/V1/menu", json={'foodname': 'Burger', 'price': '$4.99'})
            res = self.client.get("/api/V1/menu")
            order = res.get_json()
            for item in order.values():
                orders = item
            ...
            res = self.client.post("/api/V1/menu", json={'foodname': 'Burger', 'price': '$4.99'})
            self.assertEqual('Meal is already in the menu', res.get_data(as_text=True))
    ...
    def test_for_order(self):
        with self.app.test_request_context():
            self.client.post("/api/V1/menu", json={'foodname': 'Pizza', 'price': '$4.99'})
            self.client.post("/api/V1/menu", json={'foodname': 'Burger', 'price': '$7.99'})
            ...
            res = self.client.post("/api/V1/orders", json={'foodname': 'Hot dog'})
            self.assertEqual("Invalid order, the food you ordered is not in the menu", res.get_data(as_text=True))
